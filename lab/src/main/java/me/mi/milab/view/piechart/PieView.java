...
public class PieView extends View {
    ...
    private Paint descPaint = new Paint();
    ...
    private boolean touchSoll = true;//点击转动
    private boolean touchCarve = true;//点击切割
    private boolean touchBack = true;//点击返回
    ...
    private ValueAnimator animator4Back;//切割归位
    ...
    public PieView(Context context, @Nullable AttributeSet attrs) {
        ...
        animator4Back = ValueAnimator.ofFloat(1f, 1.07f);
        animator4Back.setDuration(200);
        animator4Back.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
...
//        radius = Math.min(centerX, centerY) * 0.725f;
        radius = Math.min(centerX, centerY) * 0.92f;
        if (centerCir == 1) {
        ...
        centerTextPaint.setTextSize(DesUtils.dip2px(context, 16));
    }
...
//       float  currentStartAngle = mStartAngle;                    // 当前起始角度
        ...
        for (int i = 0; i < mData.size(); i++) {
                ...
                if (pie.isCarve()) {
                    ...
                    if (mData.size() == 1) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            ...
                            RectF rectF = new RectF(centerX - mRadiusTemp, centerY - mRadiusTemp, centerX + mRadiusTemp,
                                    centerY + mRadiusTemp);
                            canvas.drawArc(rectF,angleValue + mAngle * curtFractionTouch2,
                    ...
                    if (mData.size() == 1) {
                        ...
                        float mRadiusTemp = radius * (curtFractionTouch - 1);
                        float preRadius = radius * 1.07f;
                        float preAngle = mAngle * 0.03f;
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            canvas.drawArc(centerX - preRadius + mRadiusTemp, centerY - preRadius + mRadiusTemp,
                                    centerX + preRadius - mRadiusTemp,
                                    centerY + preRadius - mRadiusTemp, angleValue + preAngle - mAngle * curtFractionTouch2,
                                    (mAngle - preAngle + mAngle * curtFractionTouch2 * 2) - 1, true, mPaint);
                        }else{
                            RectF rectF = new RectF(centerX - preRadius + mRadiusTemp, centerY - preRadius + mRadiusTemp,
                                    centerX + preRadius - mRadiusTemp,
                                    centerY + preRadius - mRadiusTemp);
                            canvas.drawArc(rectF,angleValue + preAngle - mAngle * curtFractionTouch2,
                                    (mAngle - preAngle + mAngle * curtFractionTouch2 * 2) - 1, true,mPaint);
                        }
                    }
            ...
            textPaint.setTextSize(DesUtils.dip2px(context, 14));
            //如果角度大于5度 画文字
            if(isPercentageShow){
                if(pie.getAngle() > 15){
                    Paint.FontMetrics fm = textPaint.getFontMetrics();
                    float textH = fm.descent - fm.ascent;
                    canvas.drawText(pie.getName(), pointF.x, pointF.y + textH / 4, textPaint);
                }else if(pie.getAngle() > 5){
                    textPaint.setTextSize(DesUtils.dip2px(context, 8));
                    Paint.FontMetrics fm = textPaint.getFontMetrics();
                    float textH = fm.descent - fm.ascent;
                    canvas.drawText(pie.getName(), pointF.x, pointF.y + textH / 4, textPaint);
                }
...
//            currentStartAngle += pie.getAngle();
        ...
        if (!TextUtils.isEmpty(centerText) && centerCir < 5 && centerCir >= 1) {
            ...
            float fontWidth2 = centerTextPaint.measureText(centerText2);
            canvas.drawText(centerText, rect.centerX() - fontWidth / 2, (float) (rect.centerY() - fontHeight * 0.6), centerTextPaint);
            centerTextPaint.setColor(0x999999);
            canvas.drawText(centerText2, rect.centerX() - fontWidth2 / 2, (float) (rect.centerY() + fontHeight * 0.6), centerTextPaint);
        }
        super.onDraw(canvas);
    }
...
//                            animationPostion = -1;
                            ...
                            if (touchBack) {
                                if (mData.size() > 1) {
                                    animator4Back.setStartDelay(time);
                                    animator4Back.start();
                                } else {
                                    invalidate();
                                }
